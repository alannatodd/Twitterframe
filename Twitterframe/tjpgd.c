...
static JRESULT mcu_output (
    JDEC* jd,       /* Pointer to the decompressor object */
    uint16_t (*outfunc)(JDEC*, void*, JRECT*, int*, int*),  /* RGB output function */
...
)
{
    ...
    return outfunc(jd, jd->workbuf, &rect, topOffset, leftOffset) ? JDR_OK : JDR_INTR;
}
...
)
{
    ...
    for (y = 0; y < jd->height; y += my) {      /* Vertical loop of MCUs */
        for (x = 0; x < jd->width; x += mx) {   /* Horizontal loop of MCUs */
            ...
            if (rc != JDR_OK) return rc;
            rc = mcu_output(jd, outfunc, x, y, topOffset, leftOffset); /* Output the MCU (color space conversion, scaling and output) */
            if (rc != JDR_OK) return rc;
        }
    }
